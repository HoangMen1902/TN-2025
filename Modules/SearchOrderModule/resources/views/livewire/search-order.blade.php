        ...
        <div class="max-w-4xl mx-auto mb-10 mt-10 bg-white rounded-lg shadow p-6">
            <div class="grid grid-cols-3 gap-6 mb-6 border-b pb-4 border-gray-500">
                ...
                <div>
                    ...
                    <div class="text-sm mb-1">Họ và tên: <span
                            class="font-bold">{{ (substr($result['data']['receiver_name'], 0, 3) . ' ' . str_repeat('*', 8)) ?? '...' }}</span>
                    </div>
                    <div class="text-sm mb-1">Điện thoại: <span
                            class="font-bold">{{ (substr($result['data']['receiver_phone'], 0, 3) . ' ' . str_repeat('*', 8)) ?? '...' }}</span>
                    </div>
                    <div class="text-sm">Địa chỉ: <span class="font-bold">{{ str_repeat('*', 8)}}</span></div>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr class="text-blue-700">
                            ...
                            <td class="py-2 px-3 max-w-xs break-words">
                                {{ (substr($result['data']['receiver_address'], 0, 3) . ' ' . str_repeat('*', 8)) ?? '' }}
                            </td>
