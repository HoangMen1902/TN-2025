    ...
    <div class="action-button w-full lg:py-4 hidden lg:block">
        @if (!isset($currentSku->product->published_at) || new DateTime($currentSku->product->published_at) < new DateTime())
            <form action="{{route("cart.add")}}" method="POST"" class=" flex items-center" id="addToCart">
                @csrf
                <input type="hidden" name="{{$type === "product" ? "sku_id" : "combo_id"}}"
                    value="{{$type === "product" ? $currentSku->id : $data->id}}">
                <button name="add-to-cart" value="{{$type}}" class="w-1/2 cart-add-item font-bold flex items-center justify-center sm:hidden lg:flex"><svg xmlns="http://www.w3.org/2000/svg"
                        fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                    </svg>
                    Thêm vào giỏ hàng</button>
                <button class="w-1/2 item-checkout text-white font-bold sm:hidden lg:block" name="checkout"
                    value="{{$type}}">Mua
                    ngay</button>
            </form>
        @else
            <form method="POST" class="w-full">
                ...
                <button style="width:100%; color:white"
                    class="bg-primary cart-add-item font-bold flex items-center justify-center sm:hidden lg:flex"
                    name="sku_id" value="{{$currentSku->id}}"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
