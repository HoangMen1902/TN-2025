...
<div class="">
   ...
   <div class="cart__content flex flex-col xl:flex-row justify-start items-start px-0 sm:px-1 md:px-2 xl:px-5 mt-3 md:mt-5 gap-1 md:gap-4">
            ...
            @foreach ($cartItems as $item)
                ...
                @if($type === 'sku' || $type === 'combo')
                    <div class="w-full mb-3 md:mb-5">
                        <div class="border-b p-1 md:p-2 flex flex-col md:flex-row border-gray-400 items-center md:items-stretch gap-1 md:gap-0">
                            ...
                            <div class="w-full md:w-1/7 p-1 md:p-2 flex flex-col items-center justify-center">
                                <div class="flex items-center ">
                                    ...
                                    <span class="w-8 md:w-10 text-center text-sm md:text-base">{{ $quantities[$item->id] ?? 1 }}</span>
                                    <button
                                        class="w-6 h-6 rounded-full border border-gray-500 text-base md:text-xl leading-none flex items-center justify-center hover:bg-gray-200"
                                        wire:click="increaseQuantity({{ $item->id }})">+</button>
                                </div>
                                <button wire:click="removeItem({{ $item->id }})" class="mt-1 md:mt-2 text-gray-600 hover:text-red-500">
                            ...
                            <div class="w-full md:w-1/7 text-center text-red-600 flex justify-center items-center text-sm md:text-base">
                                {{ number_format(
                                    ($type === 'sku'
        ...
        <div
            ...
            <div class="cart__summary-item flex flex-col md:flex-row md:items-center md:gap-2 mb-2 md:mb-3">
                ...
                <h3 class="font-bold text-lg md:text-xl text-red-700 ml-0 md:ml-2" wire:loading>Đang tính giá tiền..</h3>
            </div>
