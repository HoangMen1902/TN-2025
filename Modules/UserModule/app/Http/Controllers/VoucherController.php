    ...
    public function index()
{
    ...
    $vouchers = Voucher::where('expired_at', '>=', now())
        ...
        ->map(function ($voucher) use ($user) {
            ...
            return [
                'id' => $voucher->id,
                'name' => $voucher->voucher_name, // tên hiển thị
                ...
                'code' => $voucher->voucher_code, // mã áp dụng
                'terms' => "Điều kiện áp dụng: áp dụng cho đơn hàng từ " . number_format($voucher->requirement_price, 0) . " ₫. Không áp dụng cho một số sản phẩm đặc biệt.",
