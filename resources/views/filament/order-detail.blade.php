<div class="space-y-8">
    <div
        class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
        ...
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
                ...
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full font-semibold border
    ...
    : 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/50 dark:text-red-300 dark:border-red-700' }}">
            ...
            <div>
                ...
                <p class="font-semibold text-gray-800 dark:text-white">
                    {{ $paymentMethodMap[$order->paymentDetail->payment_method ?? ''] ?? 'Chưa cập nhật' }}
                </p>
            </div>
            <div>
                ...
                <p class="font-medium text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-1.5">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M18 18h1a1 1 0 001-1v-3.337a1 1 0 00-.447-.826L14.99 9.999" />
                    </svg>
                    Vận chuyển
                </p>
                <p class="font-semibold text-gray-800 dark:text-white">
                    {{ $shippingStatusMap[$order->shipping_status ?? ''] ?? 'Chưa cập nhật' }}
                </p>
            </div>
        </div>
    </div>
                                        ...
                                        <div class="flex items-center gap-4">
                                            @php
                                                ...
                                                if ($item->sku && $item->sku->images) {
                                                    if (is_array($item->sku->images) && isset($item->sku->images[0])) {
                                                        $image = is_object($item->sku->images[0]) && isset($item->sku->images[0]->path)
                                                            ? $item->sku->images[0]->path
                                                            : (is_string($item->sku->images[0]) ? $item->sku->images[0] : null);
                                                    } elseif ($item->sku->images instanceof \Illuminate\Support\Collection && $item->sku->images->first()) {
                                                        $first = $item->sku->images->first();
                                                        $image = is_object($first) && isset($first->path) ? $first->path : (is_string($first) ? $first : null);
                                                    } elseif (is_string($item->sku->images)) {
                                                        $image = $item->sku->images;
                                                    }
                                                }
                                            @endphp
                                            <img src="{{ $image ? asset('storage/' . $image) : 'https://via.placeholder.com/64x64?text=N/A' }}"
                                                alt="Ảnh sản phẩm"
                                                ...
                                                <div class="mt-1 space-x-1">
                                                    @if ($item->sku && $item->sku->skuValues)
                                                        @foreach ($item->sku->skuValues as $value)
                                                            <span
                                                                class="inline-block bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200 text-xs px-2 py-0.5 rounded-full">
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
                ...
                <div class="p-6 space-y-3">
                    <button
                        class="w-full text-center px-4 py-2 rounded-lg font-semibold text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 dark:focus:ring-indigo-800 transition">
                        Cập nhật trạng thái
                    </button>
                    <a href="{{ route('orders.print', $order->id) }}" target="_blank"
                        class="w-full block text-center px-4 py-2 rounded-lg font-semibold text-gray-800 dark:text-white bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600 transition">
                        In hóa đơn
                    </a>
                </div>
            </div>
